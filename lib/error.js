...
class CtxError extends Error{
    constructor(httpCode, errorCode, message, err) {
        super(message);
        this.httpCode = httpCode || 500;
        this.errorCode = errorCode || 'SERVER_ERROR';
        this.message = message || errorInfo[errorCode] || errorInfo['SERVER_ERROR'];
        this.err = err;
    }
    toString() {
        return `${this.httpCode} ${this.errorCode} ${this.message} ${this.err ? this.err.toString() : ''}`
    }
}
